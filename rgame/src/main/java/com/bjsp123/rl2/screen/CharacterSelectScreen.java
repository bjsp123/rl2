...
public class CharacterSelectScreen extends MenuScreen {
    ...
    @Override protected float minVirtualHeight() { return 800; }

    @Override
    protected void build(Table root) {
        Table panel = new Table();
        panel.pad(20).defaults().pad(6);
        ...
        for (int i = 0; i < classes.length; i++) {
            ...
            TextButton b = chunkyButton(c.displayName, () -> {
                selected = c;
                refreshSelection();
            ...
            classButtons[i] = b;
            panel.add(b).width(340).height(64).row();
        }
        ...
        blurb = label(selected.blurb, "dim", 1.2f);
        blurb.setWrap(true);
        ...
        panel.add(label("Seed (optional, 6 letters)", "dim", 1f)).padTop(4).row();
        TextField seedField = new TextField("", skin);
        seedField.setMaxLength(6);
        seedField.setMessageText("ABCDEF");
        panel.add(seedField).width(340).height(40).padBottom(12).row();

        panel.add(chunkyButton("Begin", () -> {
        ...
        // Back button is overlaid by framedWithBack — no inline row needed.
        com.badlogic.gdx.scenes.scene2d.ui.Stack framed =
                framedWithBack(panel, 420, 760,
